    ...
    const loadData = async () => {
        try {
            ...
            if (meRes.ok) {
                ...
                setStudent({
                    usn: data.student?.usn || '', studentName: data.student?.studentName || 'Student',
                    department: data.student?.department, semester: data.student?.semester, email: data.student?.email,
                });
            }
            ...
            <div className="bg-[#2d2d2d] border-b border-[#3d3d3d] px-5 py-3 shrink-0">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0071e3] to-[#5856d6] flex items-center justify-center text-white font-semibold text-sm shadow-lg">
                    ...
                    <div className="text-right">
                        <p className="text-xs text-[#86868b]">Academic Year 2024-25</p>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-[#2d2d2d] rounded-lg p-1 inline-flex gap-1 border border-[#3d3d3d]">
                    {(['pending', 'paid', 'history'] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                            className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === tab ? 'bg-[#0071e3] text-white shadow-sm' : 'text-[#86868b] hover:bg-[#3d3d3d]'
                                }`}>
                            {tab === 'pending' && `Pending (${unpaid.length})`}
                ...
                {activeTab === 'pending' && (
                    <div className="bg-[#2d2d2d] rounded-xl border border-[#3d3d3d] overflow-hidden">
                        <div className="px-4 py-2 border-b border-[#3d3d3d] bg-[#3d3d3d]/50">
                            <h2 className="text-xs font-semibold text-white uppercase tracking-wider">Fee Invoice</h2>
                        </div>
                        {unpaid.length === 0 ? (

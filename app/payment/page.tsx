...
function PaymentContent() {
  ...
  useEffect(() => {
    ...
    const fetchUrl = usnParam ? `/api/auth/me?usn=${usnParam}` : '/api/auth/me';
    fetch(fetchUrl)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    const handleVoiceAction = (event: CustomEvent) => {
      ...
      console.log('🎤 Voice action received in Payment Page:', type, payload);

      switch (type) {
        case 'SELECT_PAYMENT_METHOD':
          const methodMap: Record<string, number> = {
            crypto: 0, upi: 1, netbanking: 2, cash: 3
          };
          if (payload.method && methodMap[payload.method] !== undefined) {
            setCurrentMethod(methodMap[payload.method]);
          }
          break;
        case 'CONNECT_BROWSER_WALLET':
          // Trigger wallet connect - this will be handled by the crypto component
          setCurrentMethod(0); // Switch to crypto
          // Dispatch event for CryptoPayment component
          window.dispatchEvent(new CustomEvent('triggerWalletConnect', { detail: { type: 'browser' } }));
          break;
        case 'SHOW_WALLET_QR':
          setCurrentMethod(0); // Switch to crypto
          window.dispatchEvent(new CustomEvent('triggerWalletConnect', { detail: { type: 'walletconnect' } }));
          break;
        case 'INITIATE_PAYMENT':
          // Trigger payment - this will be handled by the crypto component
          window.dispatchEvent(new CustomEvent('triggerPayment', { detail: payload }));
          break;
      }
    };
